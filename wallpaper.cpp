...
void PopulateCurrentSet(WallpaperGroup* group) {
    ...
    function<void(WallpaperGroup*)> collect = [&](WallpaperGroup* g) {
        currentSet.insert(currentSet.end(), g->images.begin(), g->images.end());
        for (auto& [_, sub] : g->subgroups)
            collect(sub.get());
    };
...
void DrawUI(HDC hdc) {
    ...
    if (!backBuffer ||
        backBuffer->GetWidth() != rc.right ||
        backBuffer->GetHeight() != rc.bottom) {
        backBuffer = make_unique<Bitmap>(rc.right, rc.bottom, PixelFormat32bppARGB);
    }

    {
        ...
        if (visible && wallpaperBmp) {
            Rect r = CalcAspectFit(wallpaperBmp.get(), rc);
            g.DrawImage(wallpaperBmp.get(), r);
        }
    ...
    {
        Graphics g(hdc);
        g.DrawImage(backBuffer.get(), 0, 0);
    }
}
...
void SetupButtons() {
    ...
    buttons.push_back({
        { m, m, m + w, m + h },
        visible ? btnHideBmp.get() : btnShowBmp.get(),
        [&]() {
            visible = !visible;
            SetupButtons();
            InvalidateRect(hwndMain, nullptr, TRUE);
    ...
    buttons.push_back({
        { m + w + m, m, m + w * 2 + m, m + h },
        paused ? btnPlayBmp.get() : btnPauseBmp.get(),
        [&]() {
            paused = !paused;
            SetupButtons();
            InvalidateRect(hwndMain, nullptr, TRUE);
    ...
    buttons.push_back({
        { m, rc.bottom - h - m, m + w, rc.bottom - m },
        btnPrevBmp.get(),
        [&]() {
            if (!currentSet.empty()) {
    ...
    buttons.push_back({
        { rc.right - w - m, rc.bottom - h - m, rc.right - m, rc.bottom - m },
        btnNextBmp.get(),
        [&]() {
            if (!currentSet.empty()) {
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE, LPSTR, int) {
    ...
    LoadGroup(rootGroup.get(), L"C:\\Users\\DeclanLynch\\Desktop\\wallpaper app\\Wallpapers");
